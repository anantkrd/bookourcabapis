...
module.exports={
    getMyBookings:async(driverId,pageId)=>{
        ...
        let perPage=10;
        sqlcheck="SELECT booking.*,(select mobileNo from prayag_users where id=booking.userId ) as mobileNo FROM `prayag_booking` WHERE isDeleted='N' and (status='confirm' || status='started') and driverId=? order by id desc limit ?,?";
